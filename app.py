...
async def process_card(cc, mes, ano, cvv, site_url, variant_id=None, proxy_str=None):
    ...
    try:
        ...
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
            ...
            # ✅ IMPROVED PRICE EXTRACTION - Try multiple patterns
            subtotal = None
            patterns = [
            ...
            for start_pat, end_pat in patterns:
                subtotal = extract_between(text, start_pat, end_pat)
                if subtotal:
                    break
            if not subtotal:
                price_match = re.search(r'"price":\s*"([\d.]+)"', text)
                subtotal = price_match.group(1) if price_match else None
            if not subtotal or subtotal in ('0', '0.0', '0.00', '1', '1.0', '1.00'):
                subtotal = '0'
...
def shopify_checker():
    try:
        ...
        # ✅ IMPROVED PRICE FORMATTING + USD CONVERSION
        try:
        ...
        price_usd = round(price_float * rate, 2)

        response_data = {
            "Gateway": gateway,
            "Price": price_float,            # Original price
            "PriceUSD": price_usd,           # ✅ Converted USD
